...
export const sectionCardIconValidation = z.object({
  ...
  title: z.string().trim().min(1, "El título de la card es obligatorio").max(150, "El título es muy largo"),
  description: z.string().trim().min(1, "La descripción de la card es obligatoria"),
});

export const sectionCardsIcon = z.object({
  sectionTitle: z.string().trim().min(1, "El título de la sección es obligatorio").max(150, "El título es muy largo"),
